...
| **D36** | **The vendored `install.sh` is drift-checked, consumer-side** *(2026-07-23, #52/#53)*. §6.3's "everything outside the config block is byte-identical across consumers, and any diff is a propagation bug" was named-but-unenforced — so the D35 cosign-v3 installer shipped upstream while the vendored copies silently stayed on the v2 `.sig`/`.pem` verify path (#52), skipping signature verification on every consumer's `curl \| sh`. `release-dryrun/assert-vendored-installer.sh` (owned once, tag-pinned checkout like the D34 assert) strips the per-consumer `# ---- vendored config ----` block and asserts the rest byte-identical against the canonical installer in the same release-ci tree; each consumer runs it in its **own** CI against its repo-root copy. **Consumer-side, never upstream:** an upstream check would enumerate consumers — exactly what #29 forbids — so each consumer checks its own copy against the release-ci tag it already pins |
| **D37** | **The vendored-installer check is enforced non-bypassably at release time, and the config block is constrained** *(2026-07-23, #56)*. D36 shipped as each consumer's **own** `ci.yml installer-drift` job — consumer-authored, hence strippable: fast feedback, not enforcement. Two hardenings close that. **(1) Non-bypassable gate.** The same `assert-vendored-installer.sh` runs as a job the inherited `release.yml` `release` job **`needs:`** (canonical fetched at the executing `job.workflow_sha`, the tag the caller pins — the D34-assert pattern), so a drifted `install.sh` blocks a **signed** release. The only way past is to stop calling the pipeline, which forfeits the `rocne/release-ci` cosign identity — an unverified release is not a legit one. Consumers inherit it on their next pin bump, no downstream edit. **Residual, recorded not hidden:** this gates at **release time only** — not a drifted copy sitting on raw-on-`main` *between* releases (the file users `curl \| sh`); continuous verification of that surface is out of scope, and the per-push `ci.yml` job stays as the feedback layer. **(2) Constrained config block.** The block is diff-exempt, so its *contents* were unchecked — a place to hide arbitrary lines from the byte-identity diff. The assert now rejects any line between the markers that is not a comment, a blank, or an assignment to the three sanctioned variables (`REPO`/`BIN`/`SIGNER_REPO`, plus the canonical in-block `BIN` default): the quoted value forbids `$` and backtick (no expansion/command substitution) and the anchor forbids trailing statement-chaining. **Sound in kind (contrast #29):** both hardenings still compare downstream content against an **authoritative upstream referent** (the canonical, which the consumer cannot alter) — categorically unlike a consumer-self-assertable possession marker |
